...
This route renders the daily Takuzu puzzles: 6x6, 8x8, and 12x12, identical for every player on a given Europe/Paris calendar day (`todayParisDate` in `$lib/game/daily.ts`; was UTC, see that function's own comment for why). No backend: puzzles are pre-generated offline (`scripts/generate-daily.ts`, wrapping the vendored `takuzu-grid-factory`) and published as immutable static JSON at `/daily/<YYYY-MM-DD>.json`. `+page.ts` only fetches that file; the route holds no generator and no solution. A completed, rule-valid board is confirmed against the file's `solutionSha256`, so the answer is never sent to the client.
...
- [x] Schedule `scripts/generate-daily.ts` so tomorrow's archive is ready right at the Europe/Paris day boundary the site itself uses (`todayParisDate`). Runs as a shell job in [vps-cron](https://github.com/TomPlanche/vps-lastfm) (a small Rust cron manager, one job on this VPS among several others), at Europe/Paris midnight: `pnpm run daily:generate`, then a rebuild and `pm2 restart atkuzu` so the fresh archive file actually reaches players (`static/`'s served copy is only refreshed by a build). Previously a `node-schedule` process kept alive under pm2 just for this; ported away since vps-cron already does the DST-aware scheduling generically, for near-zero added memory instead of another idle Node process.
